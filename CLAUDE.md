...
If `docs/ai/` exists but the stub files are still empty (no real CONTEXT, no plans, README still says obsidian_inbox_watcher), this is a fresh template. Before doing any task:

1. Ask the user for project name, description, and what they're building.
2. Offer to run the renaming step (replace `obsidian_inbox_watcher` placeholders) or provide the commands.
